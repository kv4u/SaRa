      ...
      <AnimatePresence>
        {selectedTask && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
            className="w-full glass-card p-4 text-center shadow-glow-purple">
            <p className="text-[11px] text-primary-300 mb-1 font-medium">{t('roulette.your_next')}</p>
            <p className="text-base font-bold text-white">{selectedTask.title}</p>
            <p className="text-[13px] text-primary-400 mt-0.5">{selectedTask.duration} {t('dash.minutes')}</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
